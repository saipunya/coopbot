...
class UserMonthlyUsageModel {
  ...
  static async findByUserAndMonth(userId, usageMonth) {
    ...
    const [rows] = await pool.query(
      `SELECT id, user_id, usage_month, question_count, ai_preview_count, ai_preview_premium_count, last_used_at, created_at, updated_at
  ...
  static async incrementAiPreviewPremiumCount(userId, usageMonth) {
    const pool = getDbPool();
    if (!pool) {
      throw new Error("Database connection is required for monthly usage tracking.");
    }
    ...
    await pool.query(
      `INSERT INTO user_monthly_usage (
       ...
       ON DUPLICATE KEY UPDATE
         ai_preview_premium_count = ai_preview_premium_count + 1,
         last_used_at = CURRENT_TIMESTAMP,
         updated_at = CURRENT_TIMESTAMP`,
      [Number(userId || 0), String(usageMonth || "").trim()]
    );
