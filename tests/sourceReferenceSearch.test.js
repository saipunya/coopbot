...
test("managed suggested question matches group bylaw amendment before generic law lookup", async () => {
  const LawChatbotSuggestedQuestionModel = loadFresh("../models/lawChatbotSuggestedQuestionModel");
  const { findManagedSuggestedQuestionMatch } = loadFresh("../services/chatOrchestrationService");

  await LawChatbotSuggestedQuestionModel.create({
    target: "group",
    questionText: "การแก้ไขเพิ่มเติมข้อบังคับกลุ่มเกษตรกร",
    answerText: "การแก้ไขเพิ่มเติมข้อบังคับกลุ่มเกษตรกรให้ดำเนินการตามขั้นตอนที่กำหนด",
    sourceReference: "Q&A ข้อบังคับกลุ่มเกษตรกร",
    isActive: true,
  });

  const match = await findManagedSuggestedQuestionMatch(
    "การแก้ไขเพิ่มเติมข้อบังคับกลุ่มเกษตรกร",
    "group",
  );

  assert.ok(match, "expected a prepared Q&A match for group bylaw amendment");
  assert.equal(match?.target, "group");
  assert.match(match?.questionText || "", /ข้อบังคับกลุ่มเกษตรกร/);
});

test("managed suggested question matches liquidator definition questions", async () => {
  const LawChatbotSuggestedQuestionModel = loadFresh("../models/lawChatbotSuggestedQuestionModel");
  const { findManagedSuggestedQuestionMatch } = loadFresh("../services/chatOrchestrationService");

  await LawChatbotSuggestedQuestionModel.create({
    target: "coop",
    questionText: "ผู้ชำระบัญชีคือใคร",
    answerText: "ผู้ชำระบัญชีคือผู้ที่ได้รับการตั้งขึ้นเพื่อชำระสะสางกิจการของสหกรณ์หลังเลิกสหกรณ์",
    sourceReference: "Q&A การชำระบัญชี",
    isActive: true,
  });

  const match = await findManagedSuggestedQuestionMatch("ผู้ชำระบัญชีคือใคร", "coop");

  assert.ok(match, "expected a prepared Q&A match for liquidator definition");
  assert.match(match?.questionText || "", /ผู้ชำระบัญชีคือใคร/);
